...
export default function Day(props) {
  ...
  if (!dateOutOfRange || isThisDaySameAsSelectedStart || isThisDaySameAsSelectedEnd || isThisDateInSelectedRange) {
    ...
    const isCanNotSelect = customDay ? customDay(date) : false

    if (dateOutOfRange || isCanNotSelect) { // start or end date selected, and this date outside of range.
      return (
      ...
      return (
        <View style={[styles.dayWrapper, custom.containerStyle]}>
          <TouchableOpacity
            ...
            {!!(customDay && !isCanNotSelect) && (<View style={{
              position: 'absolute',
              left: 0, right: 0,
              bottom: 0,
              alignItems: 'center'
